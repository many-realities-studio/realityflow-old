...
function registerModuleListeners() {
        //console.log("[tests] RUNTIME_INITIALIZED");
        ...
        c_vxcjson = Module.cwrap('call_cb_vxcjson', null, ['string']);
        c_propertycmd = Module.cwrap('call_cb_propertycmd',null,['string']);
        c_propertycmd = Module.cwrap('call_cb_propcmd',null,['number','number','number']);
...
window.addEventListener("unity_moduleLoaded",function(){
    ...
    Module.onRuntimeInitialized = registerModuleListeners;
});
...
function run_tests_sendmessage() {
	...
	for(i=0;i<iterations;i++) {
		SendMessage('NetworkObject','TargetVWrapper');
	}
	console.timeEnd('SMv');
	...
	console.time('SMvi');
	for(i=0;i<iterations;i++) {
		SendMessage('NetworkObject','TargetViWrapper',42);
	}
	...
	console.time('SMvf');
	for(i=0;i<iterations;i++) {
		SendMessage('NetworkObject','TargetVfWrapper',42.42);
	}
	...
	console.time('SMvs');
	for(i=0;i<iterations;i++) {
		SendMessage('NetworkObject','TargetVsWrapper','This is a test string.');
	}
	...
	for(i=0;i<iterations;i++) {
		...
		SendMessage('NetworkObject','TargetVsWrapper',argument);
	}
