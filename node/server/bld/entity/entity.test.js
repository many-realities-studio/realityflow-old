...
describe('Behavior', () => {
    it("can be created", () => {
        let conn = typeorm_1.getConnection("test");
        let behavior = new behavior_1.Behavior();
    ...
    it("can be deleted", () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        behavior.Name = "behaviorName";
        conn.manager.save(behavior);
        let check = yield conn.manager.createQueryBuilder()
            .select("behavior")
            .from(behavior_1.Behavior, "behavior")
            .where("behavior.Id = :id", { id: behavior.Id })
            .getOne();
        expect(check.Id).toEqual(behavior.Id);
        yield conn.manager.createQueryBuilder()
            .delete()
            .from(behavior_1.Behavior)
            .where("Id = :id", { id: behavior.Id })
            .execute();
        let deleteCheck = yield conn.manager.createQueryBuilder()
            .select("behavior")
            .from(behavior_1.Behavior, "behavior")
            .where("behavior.Id = :id", { id: behavior.Id })
            .getOne();
